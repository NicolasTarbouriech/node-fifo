...
import { findUserByEmail, findUserById, postUser } from "../entity/user/repository/user.repository";
...
export class UserController {
  static async createUser(postedData: PostUser, res: Response) {
    try {
      ...
      const actionTypes = ['A', 'B', 'C'];

      for (const type of actionTypes) {
        user.credits[type] = calculateCredits(Number(type));
      }
